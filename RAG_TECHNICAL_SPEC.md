...
### 3.4. `retrieve_top_chunks(query, category, k=15, threshold=0.4)` (유사도 하한 필터링, 최신 지침 가중치 및 후보군 확장 검색)
...
* **로직**:
  ...
  - **API 장애 시 키워드 스코어 동적 스케일링**: 구글 API의 장애/할당량 소진 시 백업 키워드 검색을 가동하며, 매칭 단어 개수 비율을 바탕으로 유사도 점수를 `[0.4, 0.8]` 구간으로 동적 스케일링하여 일괄 고정(0.4)으로 인한 정렬 풀림을 방지합니다:
    $$\text{score} = 0.4 + \left(\frac{\text{matched\_words}}{\text{total\_query\_words}}\right) \times 0.4$$
  - **최신 지침 가중치 부스팅 (Score Boosting)**: `get_filename_from_metadata` 헬퍼를 도입하여 대괄호 유무와 관계없이 메타데이터와 파일명 내의 최신본 키워드(`(2026)`, `(최신)`, `_new` 등)를 정밀 체크합니다. 매칭 시 최종 스코어에 **보너스 점수 +0.1**을 가산합니다.
  - **맥락 보강 (Context Reinforcement) 및 파일 경계 필터링**: 검색 적중한 청크(`idx`)의 전후 청크(`idx - 1`, `idx + 1`)를 엮어 UI 컨텍스트를 구성하되, 전후 청크의 파일명이 적중 청크의 파일명과 일치할 때만 보강 텍스트에 포함시켜 다른 지침서 간의 맥락 섞임(Context Pollution)을 물리적으로 차단합니다.
  - **재정렬 (Reranking) 및 k 슬라이싱**: 가산된 최종 점수를 기준으로 검색 결과 조각들을 내림차순 정렬(Reranking)하고 상위 `k`개만 최종 채택합니다.
