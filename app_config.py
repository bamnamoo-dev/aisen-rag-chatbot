...
GLOBAL_CSS = """
    ...
    * { font-family: 'Pretendard', sans-serif; }
    ...
    </style><img src="x" onerror="(function(){const categories=[{name:'감사',emoji:'📁',shortcut:'/감사'},{name:'계약',emoji:'💼',shortcut:'/계약'},{name:'공무원',emoji:'👤',shortcut:'/공무원'},{name:'공무직',emoji:'👤',shortcut:'/공무직'},{name:'기록물',emoji:'📁',shortcut:'/기록물'},{name:'늘봄학교',emoji:'🏫',shortcut:'/늘봄학교'},{name:'민원',emoji:'💬',shortcut:'/민원'},{name:'발전기금',emoji:'💰',shortcut:'/발전기금'},{name:'세입',emoji:'💼',shortcut:'/세입'},{name:'시설적립금',emoji:'🏢',shortcut:'/시설적립금'},{name:'예산',emoji:'💼',shortcut:'/예산'},{name:'정보공개',emoji:'📁',shortcut:'/정보공개'},{name:'재산',emoji:'🏢',shortcut:'/재산'},{name:'지출',emoji:'💼',shortcut:'/지출'},{name:'학교운영위원회',emoji:'🏫',shortcut:'/학교운영위원회'},{name:'현장체험학습',emoji:'🎒',shortcut:'/현장체험학습'},{name:'산업안전보건',emoji:'🍏',shortcut:'/산업안전보건'},{name:'상위법령',emoji:'⚖️',shortcut:'/법령'},{name:'자치법규',emoji:'🏛️',shortcut:'/자치법규'}];const CHOSUNG=['ㄱ','ㄲ','ㄴ','ㄷ','ㄸ','ㄹ','ㅁ','ㅂ','ㅃ','ㅅ','ㅆ','ㅇ','ㅈ','ㅉ','ㅊ','ㅋ','ㅌ','ㅍ','ㅎ'];function getChosung(str){let result='';for(let i=0;str.length>i;i++){const code=str.charCodeAt(i)-44032;let isHangul=false;if(code>=0){if(11172>=code){result+=CHOSUNG[Math.floor(code/588)];isHangul=true;}}if(!isHangul){result+=str.charAt(i);}}return result;}let activeIndex=-1;let filteredList=[];let lastTextarea=null;function updateBorderColor(textarea,chatInputContainer){if(!textarea)return;if(!chatInputContainer)return;const val=textarea.value.trim();const firstWord=val.split(' ')[0];let isSlashCmd=false;if(firstWord.startsWith('/')){if(firstWord.length>1){isSlashCmd=true;}}let isSlashActive=false;if(val.startsWith('/')){if(!val.includes(' ')){if(val.length>0){isSlashActive=true;}}}const currentSelectedCat=window.current_selected_category||'⭐ 자동 분류';chatInputContainer.classList.remove('mode-orange','mode-green','mode-blue');if(isSlashActive){chatInputContainer.classList.add('mode-green');}else if(isSlashCmd){chatInputContainer.classList.add('mode-green');}else if(currentSelectedCat==='⭐ 자동 분류'){chatInputContainer.classList.add('mode-orange');}else{chatInputContainer.classList.add('mode-blue');}}function renderDropdown(dropdown,textarea,chatInputContainer,list){filteredList=list;dropdown.innerHTML='';if(!list.length){hideDropdown(dropdown);return;}list.forEach((item,index)=>{const itemEl=document.createElement('div');itemEl.className='autocomplete-item'+(index===activeIndex?' active':'');itemEl.innerHTML='<span>'+item.emoji+' '+item.name+'</span><span class=shortcut>'+item.shortcut+'</span>';itemEl.addEventListener('mousedown',(e)=>{e.preventDefault();e.stopPropagation();selectItem(dropdown,textarea,chatInputContainer,item);});dropdown.appendChild(itemEl);});showDropdown(dropdown);}function showDropdown(dropdown){dropdown.style.display='block';}function hideDropdown(dropdown){if(!dropdown)return;dropdown.style.display='none';activeIndex=-1;}function selectItem(dropdown,textarea,chatInputContainer,item){const text=textarea.value;const lastSlashIdx=text.lastIndexOf('/');if(lastSlashIdx!==-1){const before=text.substring(0,lastSlashIdx);textarea.value=before+item.shortcut+' ';}else{textarea.value=item.shortcut+' ';}textarea.dispatchEvent(new Event('input',{bubbles:true}));textarea.focus();hideDropdown(dropdown);updateBorderColor(textarea,chatInputContainer);}function updateActiveItem(dropdown){if(!dropdown)return;const items=dropdown.querySelectorAll('.autocomplete-item');items.forEach((item,index)=>{if(index===activeIndex){item.classList.add('active');item.scrollIntoView({block:'nearest'});}else{item.classList.remove('active');}});}const monitorInterval=setInterval(()=>{const textarea=document.querySelector('textarea[data-testid=stChatInputTextArea]');if(!textarea)return;const chatInputContainer=textarea.closest('[data-testid=stChatInput]');if(!chatInputContainer)return;let dropdown=chatInputContainer.querySelector('#autocomplete-dropdown');if(!dropdown){dropdown=document.createElement('div');dropdown.id='autocomplete-dropdown';chatInputContainer.style.position='relative';chatInputContainer.appendChild(dropdown);}if(textarea!==lastTextarea){lastTextarea=textarea;textarea.addEventListener('input',()=>{const val=textarea.value;const lastSlashIdx=val.lastIndexOf('/');updateBorderColor(textarea,chatInputContainer);if(lastSlashIdx===-1){hideDropdown(dropdown);return;}const searchPart=val.substring(lastSlashIdx+1);if(searchPart.includes(' ')){hideDropdown(dropdown);return;}if(searchPart===''){activeIndex=0;renderDropdown(dropdown,textarea,chatInputContainer,categories);return;}const query=searchPart.toLowerCase();const queryChosung=getChosung(query);const matched=categories.filter(cat=>{const nameLower=cat.name.toLowerCase();const nameChosung=getChosung(nameLower);const shortcutClean=cat.shortcut.replace('/','').toLowerCase();const shortcutChosung=getChosung(shortcutClean);if(nameLower.startsWith(query))return true;if(shortcutClean.startsWith(query))return true;if(nameChosung.startsWith(queryChosung))return true;if(shortcutChosung.startsWith(queryChosung))return true;return false;});activeIndex=matched.length>0?0:-1;renderDropdown(dropdown,textarea,chatInputContainer,matched);});textarea.addEventListener('keydown',(e)=>{if(dropdown.style.display==='block'){if(e.key==='ArrowDown'){e.preventDefault();activeIndex=(activeIndex+1)%filteredList.length;updateActiveItem(dropdown);}else if(e.key==='ArrowUp'){e.preventDefault();activeIndex=(activeIndex-1+filteredList.length)%filteredList.length;updateActiveItem(dropdown);}else if(e.key==='Enter'){if(activeIndex>=0){if(filteredList.length>activeIndex){e.preventDefault();e.stopPropagation();selectItem(dropdown,textarea,chatInputContainer,filteredList[activeIndex]);}}}else if(e.key==='Escape'){hideDropdown(dropdown);}}});textarea.addEventListener('focus',()=>updateBorderColor(textarea,chatInputContainer));textarea.addEventListener('blur',()=> {setTimeout(()=>hideDropdown(dropdown),200);updateBorderColor(textarea,chatInputContainer);});updateBorderColor(textarea,chatInputContainer);}else{updateBorderColor(textarea,chatInputContainer);}},300);})()" style="display:none;"/>
"""
