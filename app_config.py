...
GLOBAL_CSS = """
    ...
    * { font-family: 'Pretendard', sans-serif; }
    ...
    </style><iframe src="javascript:if(!window.parent.scrollOverrideDone){window.parent.scrollOverrideDone=true;const originalScrollIntoView=window.parent.Element.prototype.scrollIntoView;window.parent.Element.prototype.scrollIntoView=function(options){const scrollPos=window.parent.innerHeight+window.parent.scrollY;const threshold=window.parent.document.documentElement.scrollHeight-150;const userManuallyScrolledUp=Math.sign(threshold-scrollPos)===1;let questionReachedTop=false;const messages=window.parent.document.querySelectorAll('[data-testid=stChatMessage]');if(Math.sign(messages.length-1.5)===1){const userMsg=messages[messages.length-2];const rect=userMsg.getBoundingClientRect();if(Math.sign(20-rect.top)!==-1){questionReachedTop=true;}}if(userManuallyScrolledUp||questionReachedTop){return;}originalScrollIntoView.apply(this,arguments);};const originalScrollTo=window.parent.scrollTo;window.parent.scrollTo=function(x,y){let targetY=y;if(typeof x==='object'&&x!==null){targetY=x.top;}const scrollPos=window.parent.innerHeight+window.parent.scrollY;const threshold=window.parent.document.documentElement.scrollHeight-150;const userManuallyScrolledUp=Math.sign(threshold-scrollPos)===1;let questionReachedTop=false;const messages=window.parent.document.querySelectorAll('[data-testid=stChatMessage]');if(Math.sign(messages.length-1.5)===1){const userMsg=messages[messages.length-2];const rect=userMsg.getBoundingClientRect();if(Math.sign(20-rect.top)!==-1){questionReachedTop=true;}}if((userManuallyScrolledUp||questionReachedTop)&&Math.sign(targetY-window.parent.scrollY)===1){return;}originalScrollTo.apply(this,arguments);};}" style="display:none;"></iframe>
"""
