...
GLOBAL_CSS = """
    ...
    * { font-family: 'Pretendard', sans-serif; }
    ...
    </style><iframe srcdoc="&lt;script&gt;const parentWindow=window.parent;if(parentWindow&&!parentWindow.scrollOverrideDone){parentWindow.scrollOverrideDone=true;const originalScrollIntoView=parentWindow.Element.prototype.scrollIntoView;parentWindow.Element.prototype.scrollIntoView=function(options){const scrollPos=parentWindow.innerHeight+parentWindow.scrollY;const threshold=parentWindow.document.documentElement.scrollHeight-150;const userManuallyScrolledUp=Math.sign(threshold-scrollPos)===1;let questionReachedTop=false;const messages=parentWindow.document.querySelectorAll('[data-testid=stChatMessage]');if(Math.sign(messages.length-1.5)===1){const userMsg=messages[messages.length-2];const rect=userMsg.getBoundingClientRect();if(Math.sign(20-rect.top)!==-1){questionReachedTop=true;}}if(userManuallyScrolledUp||questionReachedTop){return;}originalScrollIntoView.apply(this,arguments);};const originalScrollTo=parentWindow.scrollTo;parentWindow.scrollTo=function(x,y){let targetY=y;if(typeof x==='object'&&x!==null){targetY=x.top;}const scrollPos=parentWindow.innerHeight+parentWindow.scrollY;const threshold=parentWindow.document.documentElement.scrollHeight-150;const userManuallyScrolledUp=Math.sign(threshold-scrollPos)===1;let questionReachedTop=false;const messages=parentWindow.document.querySelectorAll('[data-testid=stChatMessage]');if(Math.sign(messages.length-1.5)===1){const userMsg=messages[messages.length-2];const rect=userMsg.getBoundingClientRect();if(Math.sign(20-rect.top)!==-1){questionReachedTop=true;}}if((userManuallyScrolledUp||questionReachedTop)&&Math.sign(targetY-parentWindow.scrollY)===1){return;}originalScrollTo.apply(this,arguments);};const originalScrollTopSetter=Object.getOwnPropertyDescriptor(parentWindow.Element.prototype,'scrollTop').set;Object.defineProperty(parentWindow.Element.prototype,'scrollTop',{set:function(val){const scrollPos=parentWindow.innerHeight+parentWindow.scrollY;const threshold=parentWindow.document.documentElement.scrollHeight-150;const userManuallyScrolledUp=Math.sign(threshold-scrollPos)===1;let questionReachedTop=false;const messages=parentWindow.document.querySelectorAll('[data-testid=stChatMessage]');if(Math.sign(messages.length-1.5)===1){const userMsg=messages[messages.length-2];const rect=userMsg.getBoundingClientRect();if(Math.sign(20-rect.top)!==-1){questionReachedTop=true;}}if((userManuallyScrolledUp||questionReachedTop)&&Math.sign(val-this.scrollTop)===1){return;}originalScrollTopSetter.call(this,val);}});}&lt;/script&gt;" style="display:none;"></iframe>
"""
